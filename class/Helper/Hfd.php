    ...
    public function sendOrders($orderIds)
    {
        ...
        foreach ($orderIds as $orderId) {
            ...
            if ($syncFlag == self::STATUS_SEND_SUCCESS) {
                $errors[] = sprintf(__('Sync to HFD-------Already Synched with HFD-order ID: %s', HFD_WC_EPOST), $order->get_id());
                continue;
            }

            if (!$this->validateShippingMethod($order)) {
                $errors[] = sprintf(__('Sync to HFD-------order ID %s not allow sync to HFD', HFD_WC_EPOST), $order->get_id());
                continue;
            }
            ...
            if (!$result['error']) {
                ...
                $errors[] = sprintf(__('Sync to HFD-------FAIL with order ID: %s. Message: %s', HFD_WC_EPOST), $order->get_id(), $result['message']);
            }
